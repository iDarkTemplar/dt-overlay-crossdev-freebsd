...
IUSE="acpi atm audit bluetooth floppy ipv6 kerberos minimal netware nis pam ssl usb build zfs build-step2"
...
src_prepare() {
	if ! use build; then
		...
		sed -i '/BINGRP/d' "${WORKDIR}"/usr.sbin/ppp/Makefile
	fi
