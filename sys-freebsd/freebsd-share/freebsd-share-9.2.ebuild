...
pkg_setup() {
	...
	use build && mymakeopts="${mymakeopts} WITHOUT_LOCALES= "

	mymakeopts="${mymakeopts} NO_SENDMAIL= NO_MANCOMPRESS= NO_INFOCOMPRESS= "
}

REMOVE_SUBDIRS="mk termcap zoneinfo tabset syscons/scrnmaps"
...
src_unpack() {
	...
	use build || ln -s "${ROOT}/usr/src/sys" "${WORKDIR}/sys" || die "failed to set sys symlink"
}
