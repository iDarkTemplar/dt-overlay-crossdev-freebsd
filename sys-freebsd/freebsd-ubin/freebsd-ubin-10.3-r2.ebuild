...
SLOT="0"
IUSE="ar atm audit bluetooth ipv6 kerberos netware nis ssl usb build zfs noxlint build-step2"
...
pkg_setup() {
	...
	use build && mymakeopts="${mymakeopts} WITHOUT_NLS= "
...
src_prepare() {
	...
	if use build ; then
		...
		sed -i -e 's:/usr/bin/sed:sed:g' "${WORKDIR}"/usr.bin/sort/Makefile
	fi

	if use build || use noxlint ; then
		dummy_mk xlint
	fi

	if use build-step2 ; then
		dummy_mk chkey
		dummy_mk newkey
	fi

	if [ -z "$(which lint 2>/dev/null)" ] ; then
		dummy_mk xlint/llib
	fi
