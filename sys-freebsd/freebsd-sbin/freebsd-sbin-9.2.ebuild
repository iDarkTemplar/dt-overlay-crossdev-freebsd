...
# flex and bison in depend only for cross-compiling needs
# TODO: when migrate to EAPI=5-hdepend, move it to DEPEND section
...
src_prepare() {
	...
	use build && epatch "${FILESDIR}/${PN}-9.2-devd.patch"

	if use build ; then
		# Don't build gbde as it requires some build-time tools
