...
src_prepare() {
	...
	if use build ; then
		# Don't build gdbe as it requires some build-time tools
		# which can't run in cross-compilation environment
		sed -i '/[[:blank:]]gbde[[:blank:]]/d' "${WORKDIR}"/sbin/Makefile

		# Mounts require libiconv, it doesn't work yet
		sed -i '/[[:blank:]]mount_cd9660[[:blank:]]/d' "${WORKDIR}"/sbin/Makefile
		sed -i '/[[:blank:]]mount_msdosfs[[:blank:]]/d' "${WORKDIR}"/sbin/Makefile
		sed -i '/[[:blank:]]mount_udf[[:blank:]]/d' "${WORKDIR}"/sbin/Makefile

		# Missing libs
		sed -i '/[[:blank:]]setkey[[:blank:]]/d' "${WORKDIR}"/sbin/Makefile

		# Remove unsupported install option for GNU userland
		sed -i '/INSTALLFLAGS/d' "${WORKDIR}"/sbin/init/Makefile

		# Remove non-existent on Linux group
		sed -i '/BINGRP/d' \
			"${WORKDIR}"/sbin/shutdown/Makefile \
			"${WORKDIR}"/sbin/mksnap_ffs/Makefile
	fi
}
